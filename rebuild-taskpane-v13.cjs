 ...
 * UI 修复: 拖拽按钮样式、Save清除参考框、Aspect ratio用单选框样式
 */
...
// 创建只读的单选框样式显示（用于 Aspect ratio locked）
...
async function applySettings() {
  ...
  setStatus(\`Settings saved: \${targetWidthCm}×\${targetHeightCm} cm\`);
...
function buildUI() {
  ...
  // Draggable Size Setting button - 特殊样式，带图标和渐变
  ...
  refBoxBtn.innerHTML = '<span class="drag-icon">⊞</span> Draggable Size Setting';
...
function updateRefBoxButtonState(btn: HTMLButtonElement) {
  if (uiState.refBoxActive) {
    ...
    btn.innerHTML = '<span class="drag-icon">✓</span> Draggable Size Setting';
  } else {
