/**
 * PowerPoint 参考框实现
 * 使用 PowerPoint.js API 在幻灯片中插入、读取、移除参考框
 */
...
/**
 * 在 PowerPoint 幻灯片中插入参考框
 * @returns 形状名称，失败返回 null
 */
export async function insertPptReferenceBox(): Promise<string | null> {
  try {
    return await PowerPoint.run(async (context) => {
      ...
      if (slides.items.length === 0) {
        console.warn("[参考框] PowerPoint 没有选中的幻灯片");
        return null;
      }
    ...
    console.error("[参考框] PowerPoint 插入失败:", e);
    return null;
  }
}
 ...
 */
export async function getPptReferenceBoxSize(shapeName: string): Promise<ReferenceBoxSize | null> {
  try {
    return await PowerPoint.run(async (context) => {
      const slides = context.presentation.getSelectedSlides();
      slides.load("items");
    ...
    console.error("[参考框] PowerPoint 读取尺寸失败:", e);
    return null;
  }
}
 ...
 * @returns 是否成功
 */
export async function removePptReferenceBox(shapeName: string): Promise<boolean> {
  try {
    return await PowerPoint.run(async (context) => {
      const slides = context.presentation.getSelectedSlides();
      slides.load("items");
    ...
    console.error("[参考框] PowerPoint 移除失败:", e);
    return false;
  }
}

/**
 * 查找 PowerPoint 幻灯片中的参考框
 * @returns 形状名称，不存在返回 null
 */
export async function findPptReferenceBox(): Promise<string | null> {
  try {
    return await PowerPoint.run(async (context) => {
      const slides = context.presentation.getSelectedSlides();
      slides.load("items");
    ...
    console.error("[参考框] PowerPoint 查找失败:", e);
    return null;
  }
}
